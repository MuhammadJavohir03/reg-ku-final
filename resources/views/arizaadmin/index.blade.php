<x-layouts.sidebar>
    ...
    <div class="oz-wrap ariza-page">
        ...
        <div class="ariza-card">
            ...
            <div class="ariza-stepper" id="arizaStepper">
                <div class="ariza-stepper-dot" data-step="1"><span class="num">1</span><i class="bx bx-check"></i>
                </div>
                <div class="ariza-stepper-line" data-line="1"></div>
                <div class="ariza-stepper-dot" data-step="2"><span class="num">2</span><i class="bx bx-check"></i>
                </div>
                <div class="ariza-stepper-line" data-line="2"></div>
                <div class="ariza-stepper-dot" data-step="3"><span class="num">3</span><i class="bx bx-check"></i>
                </div>
                <div class="ariza-stepper-line" data-line="3"></div>
                <div class="ariza-stepper-dot" data-step="4"><span class="num">4</span><i class="bx bx-check"></i>
                </div>
            </div>
            ...
            <div class="ariza-step is-locked" id="step-2">
                ...
                <div class="ariza-search-wrap">
                    ...
                    <input type="text" id="talabaSearch" class="ariza-input ariza-input-icon"
                        placeholder="Masalan: 245 yoki Aziz Aliyev" autocomplete="off">
                </div>
        ...
        <div style="display:flex; align-items:center; gap:16px; margin-bottom:12px; flex-wrap:wrap;">
            <form method="GET" action="{{ route('ariza_admin.index') }}" class="ariza-search-wrap"
                style="max-width:460px; width:100%;">
                <i class="bx bx-search ariza-search-icon"></i>
                <input type="text" name="q" value="{{ $q }}"
                    class="ariza-input ariza-input-icon" placeholder="Talaba, fan yoki bo'lim bo'yicha qidirish"
                    autocomplete="off">
                @if ($q !== '')
                    <a href="{{ route('ariza_admin.index') }}" class="ariza-icon-btn"
                        style="position:absolute; right:8px; top:50%; transform:translateY(-50%);" title="Tozalash">
                        <i class="bx bx-x"></i>
                    </a>
                @endif
            </form>
                ...
                <tbody>
                    @forelse ($arizalar as $index => $ariza)
                        <tr>
                            ...
                            <td class="text-center">
                                <form
                                    action="{{ route('ariza_admin.destroy', ['type' => strtolower($ariza->maktab_turi), 'ariza_admin' => $ariza->id]) }}"
                                    method="POST" onsubmit="return confirm('Arizani o\'chirishni tasdiqlaysizmi?');">

                                    @csrf
                                    @method('DELETE')

                                    <button type="submit" class="ar-btn" style="color:#dc3545;" title="O'chirish">
                    ...
                    @empty
                        ...
                        <tr>
                            <td colspan="9" style="text-align:center;padding:35px;color:#888;">
                                Arizalar topilmadi.
                            </td>
                        </tr>
                    @endforelse
    ...
    <style>
        .ariza-page {
            --ariza-primary: #4f46e5;
            --ariza-primary-light: #eef2ff;
            --ariza-success: #16a34a;
            --ariza-success-light: #f0fdf4;
            --ariza-danger: #dc2626;
            --ariza-danger-light: #fef2f2;
            --ariza-warn: #d97706;
            --ariza-warn-light: #fffbeb;
            --ariza-border: #e5e7eb;
            --ariza-text-muted: #8a8f98;
        }
        ...
        .ariza-stepper-dot.is-active {
            background: var(--ariza-primary);
            color: #fff;
            box-shadow: 0 0 0 5px rgba(79, 70, 229, .15);
        }

        .ariza-stepper-dot.is-done {
            background: var(--ariza-success);
            color: #fff;
        }

        .ariza-stepper-dot.is-done .num {
            opacity: 0;
        }

        .ariza-stepper-dot.is-done i {
            opacity: 1;
        }
        ...
        .ariza-step-badge {
            ...
            height: 20px;
            border-radius: 6px;
            background: var(--ariza-primary-light);
            color: var(--ariza-primary);
            font-size: 11px;
            display: inline-flex;
            align-items: center;
            justify-content: center;
        ...
        .ariza-input {
            ...
            max-width: 380px;
            padding: 11px 14px;
            border: 1.5px solid var(--ariza-border);
            border-radius: 10px;
            font-size: 13.5px;
            color: #1f2430;
            background: #fff;
        ...
        .ariza-input:focus {
            border-color: var(--ariza-primary);
        ...
        .user-result-item .urn {
            font-weight: 600;
            color: #1f2430;
        }

        .user-result-item .ure {
            color: var(--ariza-text-muted);
            font-size: 12px;
            margin-top: 1px;
        }
        ...
        .ariza-toggle-btn.is-selected i {
            color: var(--ariza-primary);
        }
        ...
        .ariza-grade-chip.is-total {
            background: var(--ariza-primary-light);
            border-color: #dfe3fb;
        }

        .ariza-grade-chip.is-total .value {
            color: var(--ariza-primary);
        }

        .ariza-grade-chip.is-low .value {
            color: var(--ariza-danger);
        }

        .ariza-grade-chip.is-ok .value {
            color: var(--ariza-success);
        }
        ...
        .ariza-empty-grade {
            ...
            gap: 12px;
            background: var(--ariza-warn-light);
        ...
        .ariza-submit-btn:hover:not(:disabled) {
            transform: translateY(-1px);
            box-shadow: 0 10px 22px rgba(79, 70, 229, .35);
        }

        .ariza-submit-btn:disabled {
            opacity: .4;
            cursor: not-allowed;
            box-shadow: none;
        }
        ...
        try {
            (function() {
                ...
                function resetFromMaktab() {
                    ...
                    maktabToggle.querySelectorAll('.ariza-toggle-btn').forEach(function(b) {
                ...
                function searchUsers(q) {
                    fetch(`{{ route('ariza_admin.search_user') }}?q=${encodeURIComponent(q)}`)
                        ...
                        .catch(function() {
                            userResults.innerHTML =
                                '<div style="padding:12px; color:#dc2626; font-size:13px;">Xatolik yuz berdi</div>';
                ...
                function renderUserResults(users) {
                    if (!users.length) {
                        userResults.innerHTML =
                            '<div style="padding:12px; color:#9aa0ac; font-size:13px; text-align:center;">Topilmadi</div>';
                        userResults.style.display = 'block';
                        return;
                    }

                    userResults.innerHTML = users.map(function(u) {
                        return '<div class="user-result-item" data-id="' + u.id + '" data-name="' + (u.name ||
                                '') + '" data-email="' + (u.email || '') + '">' +
                            '<div class="urn">' + (u.name || '—') + ' <span style="color:#c2c6ce;">#' + u.id +
                            '</span></div>' +
                ...
                function checkGrade(userId, subjectId) {
                    ...
                    bahoPreviewCont.innerHTML =
                        '<div style="color:#9aa0ac; font-size:13px; padding:6px 0;">Tekshirilmoqda...</div>';
                    ...
                    fetch(`{{ route('ariza_admin.check_grade') }}?user_id=${userId}&subject_id=${subjectId}`)
                        ...
                        .then(function(data) {
                            renderBahoPreview(data);
                        })
                        .catch(function() {
                            bahoPreviewCont.innerHTML =
                                '<div style="color:#dc2626; font-size:13px; padding:6px 0;">Xatolik yuz berdi</div>';
                        });
                }
